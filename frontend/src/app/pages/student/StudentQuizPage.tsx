...
export const StudentQuizPage = () => {
    ...
    if (isLoading) {
        return <div className="flex justify-center items-center h-screen font-bold text-gray-500">문제를 불러오는 중...</div>;
    }

    if (quizzes.length === 0) {
        return (
            <div className="max-w-3xl mx-auto p-6 mt-10 text-center">
                <h2 className="text-2xl font-bold text-gray-800">등록된 문제가 없습니다.</h2>
                <button onClick={() => navigate("/student/my-page")} className="mt-4 text-blue-600 font-bold hover:underline">
                    마이페이지로 돌아가기
                </button>
            </div>
        );
    ...
    return (
        <div className="max-w-3xl mx-auto p-6 mt-6">
            ...
            <div className="flex items-center justify-between mb-8">
                <button onClick={() => navigate("/student/my-page")} className="flex items-center text-gray-500 hover:text-gray-800 font-bold transition">
            ...
            <div className="bg-white p-8 rounded-3xl shadow-sm border border-gray-200 min-h-[400px] flex flex-col">
                ...
                {currentQuiz.questImagePath && (
                    <div className="mb-8 rounded-xl overflow-hidden border border-gray-200">
                        <img src={currentQuiz.questImagePath} alt="문제 이미지" className="w-full max-h-64 object-contain bg-gray-50" />
                    </div>
                )}
                        ...
                        <div className="flex gap-6 justify-center">
                            <button
                                ...
                                className={`w-32 h-32 rounded-2xl border-4 text-6xl font-black transition-all ${
                                    userAnswers[currentIndex] === 0 ? "border-blue-500 bg-blue-50 text-blue-600 shadow-lg scale-105" : "border-gray-200 hover:border-gray-300 text-gray-400"
                                }`}
                            >O</button>
                            <button
                                onClick={() => handleSelectAnswer(1)} // 1을 X로 간주
                                className={`w-32 h-32 rounded-2xl border-4 text-6xl font-black transition-all ${
                                    userAnswers[currentIndex] === 1 ? "border-red-500 bg-red-50 text-red-600 shadow-lg scale-105" : "border-gray-200 hover:border-gray-300 text-gray-400"
                                }`}
                            >X</button>
                        </div>
                    )}
            ...
            <div className="flex justify-between items-center mt-8">
                <button
                    ...
                    className={`flex items-center px-6 py-3 rounded-xl font-bold transition-colors ${
                        currentIndex === 0 ? "text-gray-300 cursor-not-allowed" : "bg-white border border-gray-300 text-gray-700 hover:bg-gray-50"
                    }`}
                >
                ...
                {isLastQuestion ? (
                    ...
                    >
                        {isSubmitting ? "제출 중..." : "최종 제출하기"} <CheckCircle size={20} className="ml-2" />
                    </button>
                ) : (
                    <button
                        onClick={() => setCurrentIndex(prev => prev + 1)}
